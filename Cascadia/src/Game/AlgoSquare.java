...
public class AlgoSquare {
  ...
  private static AnimalCard parameter(Scanner scan) {
    int enter;
  ...
  private static void overPopulation(HashMap<TilesSquare, WildlifeToken> pickaxe, int numberDiscard, HashMap<WildlifeToken, Integer> wildlifeTokens) {
  ...
  private static void isOverpopulation(HashMap<TilesSquare, WildlifeToken> pickaxe, Scanner scan, HashMap<WildlifeToken, Integer> wildlifeTokens) {
    ...
    for(var elem: valueCount.values()){
      if(elem == 3) {
        ...
        if(enter == 1) {
          overPopulation(pickaxe, 3, wildlifeTokens);
          break;
        }
      }
      if(elem == 4) {
        System.out.println("4 jetons faunes identiques, déclenchement de la sûrpopulation");
        overPopulation(pickaxe, 4, wildlifeTokens);
        break;
      }
    }
  }
  
  private static HashMap<TilesSquare, WildlifeToken> pickPickaxe(HashMap<TilesSquare, WildlifeToken> pickaxe, Scanner scan, HashMap<WildlifeToken, Integer> wildlifeTokens) {
    var picked = new HashMap<TilesSquare, WildlifeToken>();
    var enter = -1;
    var arrayPickaxe = pickaxe.keySet().toArray();
    printPickaxe(pickaxe);
    isOverpopulation(pickaxe, scan, wildlifeTokens);
  ...
  private static void makeWildlifeMove(HashMap<Coordonate, TilesSquare> player, WildlifeToken wildlife, Scanner scan, HashMap<WildlifeToken, Integer> wildlifeTokens) {
  ...
  private static void makeMove(HashMap<Coordonate, TilesSquare> player, WildlifeToken wildlife, TilesSquare tile, Scanner scan, HashMap<WildlifeToken, Integer> wildlifeTokens) {
    printPlayer(player);
    System.out.println("----------------------------");
    makeTileMove(player, tile, scan);
    printPlayer(player);
    System.out.println("----------------------------");
    makeWildlifeMove(player, wildlife, scan, wildlifeTokens);
  }
  
  private static void loopAlgorythmSquare(HashMap<TilesSquare, WildlifeToken> pickaxe, HashMap<Coordonate, TilesSquare> player1, HashMap<Coordonate, TilesSquare> player2, List<TilesSquare> tiles, HashMap<WildlifeToken, Integer> wildlifeTokens, Scanner scan, AnimalCard card) {
    var picked = new HashMap<TilesSquare, WildlifeToken>();
    for(var i = 0; i < 5; i++) {
      ...
      picked = pickPickaxe(pickaxe, scan, wildlifeTokens);
      craftPickaxe(pickaxe, tiles, wildlifeTokens, picked.keySet().iterator().next(), null);
      makeMove(player1, picked.get(picked.keySet().iterator().next()), picked.keySet().iterator().next(), scan, wildlifeTokens);
      ...
      picked = pickPickaxe(pickaxe, scan, wildlifeTokens);
      craftPickaxe(pickaxe, tiles, wildlifeTokens, picked.keySet().iterator().next(), null);
      makeMove(player2, picked.get(picked.keySet().iterator().next()), picked.keySet().iterator().next(), scan, wildlifeTokens);
    }
  }
  ...
  private static void countPoints(AnimalCard card, HashMap<Coordonate, TilesSquare> player1, HashMap<Coordonate, TilesSquare> player2) {
  ...
  public static void algorythmSquare() {
    ...
    loopAlgorythmSquare(pickaxe, player1, player2, tiles, wildlifeTokens, scan, card);
    scan.close();
    countPoints(card, player1, player2);
  }
